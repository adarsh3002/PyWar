...
# nebula_image = simplegui.load_image(https://img.freepik.com/free-photo/galaxy-nature-aesthetic-background-starry-sky-mountain-remixed-media_53876-126761.jpg?w=900&t=st=1684579836~exp=1684580436~hmac=cce66e47d1163379e8e5a566f5b31af6ed57206a092a95e4f4b2f3880fa69665")

# splash image
splash_info = ImageInfo([200, 150], [400, 300])
splash_image = simplegui.load_image(
    "https://i.ibb.co/Wnykw9J/Py-War-Battle-in-Space-1.png")
...
# asteroid_image = simplegui.load_image("https://static.wikia.nocookie.net/tastyplanet/images/1/16/Metera.png/revision/latest?cb=20211111182918")
...
def group_collide(s_group, other_object):
    ...
    for x in set(s_group):
        if (x.collide(other_object)):
            a_explosion = Sprite(x.get_position(), [
...
class Ship:
    ...
    def shoot(self):
        ...
        a_missile = Sprite(missile_pos, missile_vel, self.angle,
...
def rock_spawner():
    ...
    if started:
        if len(rock_group) != 12:
            rock_pos = [random.randrange(
                0, WIDTH), random.randrange(0, HEIGHT)]

            if not (dist(my_ship.get_position(), rock_pos) <= 10):
                rock_vel = [(random.random() * .6 - .3) + score /
                            5.0, (random.random() * .6 - .3) + score/5.0]
...
frame = simplegui.create_frame("PYWAR: Battle In Space", WIDTH, HEIGHT, 200)
